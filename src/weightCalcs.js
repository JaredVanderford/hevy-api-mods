...
export const calculateWeight = (weight_kg, increment = 0) => {
    ...
    return (Math.round(outWeight * 100000) / 100000).toFixed(5);
};
export const warmupWeight = (weight) => weight * .5;
export const convertWeight = (weight) => {
    const { KG_MULTIPLIER: kgMultiplier, USE_LBS: useLbs } = process.env;
    return Math.round((weight/(useLbs ? kgMultiplier : 1)*100)/100).toFixed(2);
};

export const formatWeight = (weight) => (Math.round(weight * 100000) / 100000).toFixed(5);